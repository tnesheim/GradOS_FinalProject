...
#define LED_CONNECT LED_1

#define IS_SRVC_CHANGED_CHARACT_PRESENT      0                                          /**< Include or not the service_changed characteristic. if not enabled, the server's database cannot be changed for the lifetime of the device*/

#define DEVICE_NAME                          "Nunchuck_Node"                               /**< Name of device. Will be included in the advertising data. */
...
static ble_nunchuck_t m_ble_nunchuck;
...
static void setLED(uint32_t led_num, bool set_led);
...
static void on_ble_evt(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        case BLE_GAP_EVT_CONNECTED:
            m_ble_nunchuck.conn_handle = p_ble_evt->evt.gap_evt.conn_handle;
            break;
...
static void advertising_init(void)
{
    ...
    {
        {BLE_NUNCHUCK_SERVICE_UUID, BLE_UUID_TYPE_VENDOR_BEGIN}
    };
...
static void services_init(void)
{
   ...
   //Initialize the Nunchuck service
   err_code = ble_nunchuck_init(&m_ble_nunchuck);
   APP_ERROR_CHECK(err_code);
}
...
static void conn_params_init(void)
{
    ...
    cp_init.start_on_notify_cccd_handle    = m_ble_nunchuck.nunchuck_char_handles.cccd_handle;
...
static void initLEDS()
{
   ...
   nrf_gpio_cfg_output(LED_CONNECT);
   nrf_gpio_pin_clear(LED_ERROR);
   nrf_gpio_pin_clear(LED_CONNECT);
}

/*Set or clear the specified LED*/
static void setLED(uint32_t led_num, bool set_led)
{
   if(set_led)
   {
...
int main(void)
{
   ...
   for(;;)
   {
      ...
      //Write the Nunchuck data to the Nunchuck Characteristic and notify central
      ble_nunchuck_send(&m_ble_nunchuck, &nunData);
   }
}